...
import com.example.echoloc.Directions.BitmapConverter
            ...
            for (location in locationResult.locations) {
                if (location != null) {
                    val locationModel = LocationModel(pref.getData("id"), pref.getData("name"),
                        pref.getData("call"), location.latitude, location.longitude, pref.getData("profile").toString())
...
//                        val bitmapIcon = createMarkerIcon(true, null, Uri.parse(locationModel.profileImageUrl)) // 불러와서 넣기
...
//                        markerItem.icon = bitmapIcon
    ...
    private fun setBalloonView(marker: TMapMarkerItem, name: String) {
        ...
        if (marker.canShowCallout) {
            ...
            val bitmap = createMarkerIcon(false, R.drawable.direct, null)
            marker.calloutRightButtonImage = bitmap
        }
    }

    private fun createMarkerIcon(isUrl: Boolean, image: Int?, imageUri: Uri?): Bitmap {
